...
//get the list
	...
	//############################################################
			//get the extensions
			$sqle = "select extension,number_alias,effective_caller_id_name,effective_caller_id_number,user_context from v_extensions ";
			//$sqle .= "where 1 = 1 ";
			//if ($_GET['show'] == "all" && permission_exists('extension_all')) {
				//show all gateways
			//} else {
				$sqle .= "where domain_uuid = '$domain_uuid' ";
			//}
			//$sqle .= $sql_search; //add search mod from above
			//if (strlen($order_by) > 0) {
			//	$sqle .= ($order_by == 'extension') ? "order by $order_text ".$order." " : "order by ".$order_by." ".$order." ";
			//}
			//else {
			//	$sqle .= "order by $order_text ";
			//}
			//	$sqle .= "limit $rows_per_page offset $offset ";
			$prep_statemente = $db->prepare(check_sql($sqle));
			$prep_statemente->execute();
			$extensions = $prep_statemente->fetchAll(PDO::FETCH_NAMED);
			
				//var_dump($sqle);
			
			unset ($prep_statemente, $sqle);
			
			//var_dump($extensions[0]["extension"]."@".$extensions[0] ["user_context"]);
...
//show the error message or show the content
	if (strlen($msg) > 0) {
		...
		//order the array
			...
			$check = false;
			foreach($registrations as $row) {
				if(count($temp) == 0) array_push($temp,$row);
				foreach($temp as $trow){
					if($row['user'] === $trow['user']) $check = true;
				}
				if($check == false) array_push($temp,$row);
				//var_dump(in_array($row['user'], $temp));
				$check = false;
			}
			//var_dump($temp);
			$registrations = $temp;
			
			//var_dump($extensions[0]["effective_caller_id_name"]);
			$tempr = array();
			$tempe = array();
			foreach($registrations as $row){
				array_push($tempr,$row['user']);
			}
			
			foreach($extensions as $row){
				array_push($tempe,$row["extension"]."@".$row["user_context"]);
			}
			
			//var_dump($tempr);
			//var_dump($tempe);
			
			$result = array_diff($tempe,$tempr);
			
			function getExtName($extensions,$rowe){
				foreach($extensions as $row){
					if($row["extension"]."@".$row["user_context"] == $rowe) return $row["effective_caller_id_name"];
				}
			}
			
			$final = array();
			foreach($result as $row){
				foreach($extensions as $rowe){
					if($rowe["extension"]."@".$rowe["user_context"] == $row){
						array_push($final,array("user" => $row, "contact" => $rowe["effective_caller_id_name"]));
					}
				...
				foreach ($registrations as $row) {
					...
					//search 
						...
						if ($matches != FALSE) {
							...
							//show the registrations
								echo "<tr>\n";
								echo "	<td valign='top' class='".$row_style[1]." tr_link_void' style='vertical-align:middle; display: table-cell; align: center;'>\n";
								...
								echo "	</td>\n";
								echo "	<td class='row_style1'>".escape($row['user'])."&nbsp;</td>\n";
								//echo "	<td class='".$row_style[$c]."'>".escape($row['agent'])."&nbsp;</td>\n";
								echo "	<td class='row_style1'>".escape(explode('"',$row['contact'])[1])."</td>\n";
								...
								echo "	<td class='row_style1'>CONNECTED!</td>\n";
								echo "	<td class='row_style1' style='text-align: right;' nowrap='nowrap'>\n";
				...
				foreach ($final as $row) {
					...
					$matches = preg_grep ("/$search/i",$row);
						if ($matches != FALSE) {
								...
								//show the extensions not registered
								echo "<tr >\n";
								echo "	<td valign='top' class='"."row_style0"." tr_link_void' style='vertical-align:middle; display: table-cell; align: center;'>\n";
								//echo "		<input type='checkbox' name=\"registrations[$x][checked]\" id='checkbox_".$x."' value='true' onclick=\"if (!this.checked) { document.getElementById('chk_all_".escape($row['user'])."').checked = false; }\">\n";
								//echo "		<input type='hidden' name=\"registrations[$x][user]\" value='".escape($row)."' />\n";
								//echo "		<input type='hidden' name=\"registrations[$x][profile]\" value='".escape($row['sip_profile_name'])."' />\n";
								//echo "		<input type='hidden' name=\"registrations[$x][agent]\" value='".escape($row['agent'])."' />\n";
								//echo "		<input type='hidden' name=\"registrations[$x][host]\" value='".escape($row['host'])."' />\n";
								//echo "		<input type='hidden' name=\"registrations[$x][domain]\" value='".escape($row['sip-auth-realm'])."' />\n";
								echo "	</td>\n";
								echo "	<td class='row_style0'>".escape($row['user'])."&nbsp;</td>\n";
								//echo "	<td class='".$row_style[$c]."'>".escape($row['agent'])."&nbsp;</td>\n";
								echo "	<td class='row_style0'>".escape($row["contact"])."</td>\n";
								//echo "	<td class='row_style0'>".escape(getExtName($extensions,$row))."</td>\n";
								//echo "	<td class='".$row_style[$c]."'><a href='https://".escape($row['lan-ip'])."' target='_blank'>".escape($row['lan-ip'])."</a></td>\n";
								//echo "	<td class='".$row_style[$c]."'><a href='https://".escape($row['network-ip'])."' target='_blank'>".escape($row['network-ip'])."</a></td>\n";
								//echo "	<td class='".$row_style[$c]."'>".escape($row['network-port'])."</td>\n";
								//echo "	<td class='".$row_style[$c]."'>".escape($row['host'])."</td>\n";
								...
								//echo "	<td class='".$row_style[$c]."'>".escape($row['ping-time'])."</td>\n";
								//echo "	<td class='".$row_style[$c]."'>".escape($row['sip_profile_name'])."</td>\n";
								echo "	<td class='row_style0'>DESCONECTED!</td>\n";
								echo "	<td class='row_style0' style='text-align: right;' nowrap='nowrap'>\n";
								//echo "		<button type='submit' class='btn-default' name=\"registrations[$x][action]\" value='unregister'>".$text['button-unregister']."</button>\n";
								//echo "		<button type='submit' class='btn-default' name=\"registrations[$x][action]\" value='provision'>".$text['button-provision']."</button>\n";
								//echo "		<button type='submit' class='btn-default' name=\"registrations[$x][action]\" value='reboot'>".$text['button-reboot']."</button>\n";
								echo "	</td>\n";
