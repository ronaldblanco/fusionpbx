...
//get the call activity
	...
	if (is_array($activity)) {
		...
		//**************************************************
		//Automatic transfer after 5 minutes in hold
		$dest_to_transfer = $_SESSION['mycallcenter_ext'];
		$time_to_tranfer = $_SESSION['mycallcenter_time'];
		$context_to_tranfer = $_SESSION['domain_name'];
		if($fields['callstate'] == "HELD" && $fields['variable_sip_profile_name'] == "internal" && ($fields['context'] == $context_to_tranfer || $fields['Other-Leg-Context'] == $context_to_tranfer)){ //if duration mayor of 5 and call in HOLD and context, tranfer the call
			//$cmd = "help";
			$onhold = true;
			//var_dump($_SESSION['fs_trans']);
			$extension_msg = $fields["extension"];
			$caller = $fields['Caller-Caller-ID-Name']. " number " .$fields['Caller-Caller-ID-Number'];
			$time = (time() - strtotime($fields['created']))/60;
			if ($time > $time_to_tranfer) {
				$fp = event_socket_create($_SESSION['event_socket_ip_address'], $_SESSION['event_socket_port'], $_SESSION['event_socket_password']);
				$transfering = ", executing transfering of the call";
				$cmd = "uuid_transfer ". $fields['uuid'] ." -bleg " . $dest_to_transfer; //transfer to extention
				if($fp){
					$myresults = trim(event_socket_request($fp, 'api '.$cmd));
				}
			}
		}
		//***************************************************
...
//**************************************************
echo "<center><h6>Automatic tranfer it is activate in domain ".$context_to_tranfer." for calls in hold for more than ".$time_to_tranfer." minutes and will be send to extension ".$dest_to_transfer."!</h6></center>";
//***************************************************
...
if($onhold == true){ 
	...
	echo "Extension ". $extension_msg . " have a call from " .$caller. " On Hold for ".$time." minutes".$transfering."!";
	echo "		</td>";
	echo "	</tr>";
}
//***********************************************
