...
//check permissions
	...
	if (permission_exists('bdr_status_view')) {
		//access granted
	}
	else {
		echo "access denied";
...
//additional includes
	...
	$is_show_fusion_bdr = filter_var($_SESSION['server']['bdr_fusionpbx_enable']['boolean'], FILTER_VALIDATE_BOOLEAN);
	$is_show_freeswitch_bdr = filter_var($_SESSION['server']['bdr_freeswitch_enable']['boolean'], FILTER_VALIDATE_BOOLEAN);

	if ($is_show_fusion_bdr) {
	//show the header
	...
	//table headers
		...
		foreach ($result as $row) {
			...
			if (permission_exists('bdr_status_delete')) {
