...
//if the connnection is available then run it and return the results
	if (!$fp) {
		...
		//show content
			...
			//***************************************
			echo "<h6>Automatic tranfer it is activate in domain ".$context_to_tranfer." for calls in hold for more than ".$time_to_tranfer." minutes and will be send to extension ".$dest_to_transfer."!</h6>";
			//***************************************
			echo "<br /><br />\n";
		...
		//show the results
			...
			echo "	<th>Call State</th>\n";
			echo "	<th>Initial IP Addr</th>\n";
			if (permission_exists('call_active_hangup')) {
			...
			if (is_array($rows)) {
				...
				foreach ($rows as &$row) {
					...
					//send the html
						...
						echo "	<td>".escape((time() - strtotime($created))/60 ." min")."&nbsp;</td>\n"; //Duration
						...
						$time_to_tranfer = $_SESSION['mycallcenter_time'];
						$context_to_tranfer = $_SESSION['domain_name'];*/
						if((time() - strtotime($created))/60 > 1 && $callstate == "HELD" && $sip_profile == "internal" && $domain_name == $context_to_tranfer){ //if duration mayor of 5 and call in HOLD in contex, call will be transfered
							$time = (time() - strtotime($fields['created']))/60;
							if ($time > $time_to_tranfer) {
								$cmd = "uuid_transfer ". $uuid ." -bleg " . $dest_to_transfer; //transfer to extention
								//$fp = event_socket_create($_SESSION['event_socket_ip_address'], $_SESSION['event_socket_port'], $_SESSION['event_socket_password']);
						...
						echo "	<td>".escape($callstate)."&nbsp;</td>\n"; //Call state
						echo "	<td>".escape($initial_ip_addr)."&nbsp;</td>\n";
						if (permission_exists('call_active_hangup')) {
